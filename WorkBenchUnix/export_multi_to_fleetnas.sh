#!/bin/bash
# Created: 2026-08-03 UTC — pushes Immich export_multi from WBU to FleetNAS (UGREEN
# DXP6800 Pro, RAID 5 / Btrfs), then verifies. Runs ON WorkBenchUnix. One-way push.
#
# The FleetNAS counterpart to export_multi_to_macmini.sh. Structure is borrowed from
...
DEST_RSYNC="$DEST_HOST:immich/export_multi/"
DEST_PATH="/volume1/immich/export_multi"
DEST="$DEST_RSYNC"

# export_multi is regenerated wholesale by export_archive.py (~11hr, manual-only), so a
...
LOG_FILE="$LOG_DIR/fleetnas_export_multi_${TS}.log"
WORK_DIR="$LOG_DIR/verify-work-export-multi"
...
log "=== Starting export_multi sync: WBU -> FleetNAS ==="
...
log "Syncing export_multi (--delete, --max-delete=$MAX_DELETE)..."
set +e
...
if [ "$DRIFT_COUNT" -gt 0 ]; then
    ...
    log "export_multi verification complete — FleetNAS matches WBU exactly (0 differences)."
fi

SRC_COUNT=$(find "$SRC" -type f | wc -l)
log "Source file count: $SRC_COUNT"

log "=== export_multi sync to FleetNAS complete ==="

# --- Cron placeholder — NOT ACTIVE, comment-only quick-reference ---
# Manual-only by design, matching export_multi_to_macmini.sh: export_multi only changes
# when export_archive.py is re-run by hand (~11hr), so there is nothing for a nightly
# job to pick up most nights.
# /home/dhm/repos/scripts/WorkBenchUnix/export_multi_to_fleetnas.sh >> /home/dhm/.cache/fleetnas-sync/cron.log 2>&1
