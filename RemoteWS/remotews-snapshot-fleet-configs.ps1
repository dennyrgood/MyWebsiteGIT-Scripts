...
# Created 2026-07-21; converted to repo-based snapshot 2026-07-23 after remotews
# gained a local repo checkout (was previously a no-repo box staging bespoke files
# + Task Scheduler exports through OneDrive\ForFleetConfigs\remotews\).
#
...
$Machine = "RemoteWS"         # fleet-configs folder name for this box
...
Get-ScheduledTask | Where-Object {
    ($_.Actions | ForEach-Object { "$($_.Execute) $($_.Arguments)" }) -match 'fleet[-_ ]?metrics[-_ ]?server|fleet[-_ ]?monitor|heartbeat[-_ ]?writer|run[-_ ]?hidden|watchdog|power[-_ ]?heartbeat|tailscale[-_ ]?watchdog|syncthing|stctl'
